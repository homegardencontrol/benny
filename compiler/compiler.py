...
for line in lines:
    ...
    if not __first__line__:
        for i,c in enumerate(line):
            if c.isspace():
                ...
                break
    if line.find('#')>=0 :
        __benny__syntax__ = line[0:line.find('#')].strip()
        if __benny__syntax__=='':
            if line.find('#')>=__indent__:
                out_text = out_text +line[__indent__:]
            else:
                out_text = line
            continue
    else:
    ...
    if __benny__syntax__.find('.')<0 and len(__benny__syntax__.split('['))==2 and __benny__syntax__.find('=')>__benny__syntax__.find('[') and __benny__syntax__.find('=')<__benny__syntax__.find(']'):
        ...
        try: # call exist class
            exec('test = '+__benny__object__)
            out_text = out_text +line[__indent__:__line__indent__]+ __benny__syntax__+'\n'
            continue
        except:
            if not __benny__object__ in __benny__class__:
                ...
                out_text = out_text+line[__indent__:__line__indent__]+' __benny__list__=[]\n'
                out_text = out_text+line[__indent__:__line__indent__]+' pass\n'
    ...
    if __benny__syntax__.find('.')<__benny__syntax__.find('=') and __benny__syntax__.find('.')>=0:
        ...
        if __benny__syntax__[0:__benny__syntax__.find('=')].find('(')>=0:
            ...
            out_text = out_text +line[__indent__:__line__indent__]+ __benny__syntax__
            continue
        #print(__benny__syntax__)
        if __benny__syntax__.find('[')>=0:
            __benny__object__list__ = __benny__syntax__[0:__benny__syntax__.find('[')].split('.')
            __benny__object__list__[len(__benny__object__list__)-1] = __benny__object__list__[len(__benny__object__list__)-1]+__benny__syntax__[__benny__syntax__.find('['):]
        else:
            if __benny__syntax__.find('=')>=0:
                __benny__object__list__ = __benny__syntax__[0:__benny__syntax__.find('=')].split('.')
                __benny__object__list__[len(__benny__object__list__)-1] = __benny__object__list__[len(__benny__object__list__)-1]+__benny__syntax__[__benny__syntax__.find('='):]
        ...
        #print(__benny__object__list__)
        try: # call exist class
            exec('test = '+__benny__object__list__[0])
            out_text = out_text +line[__indent__:__line__indent__]+ __benny__syntax__+'\n'
            continue
        except:
            pass
